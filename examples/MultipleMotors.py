...
# Print the responses
print(f"------- Set X axis command responses -------")
...
# Get the responses - look for both responses to be returned before continuing
responses = yaxis.get_all_responses()
while not all(y in responses for y in ["RI02CY*", "CI02CY*"]):
    responses = responses + yaxis.get_all_responses()

# Print the responses
print(f"------- Set Y axis command responses -------")
yaxis.parse_responses(responses)
...
responses = xaxis.get_all_responses()
while not all(x in responses for x in ["RI00SX*", "RI00SY*", "CI00SX*", "CI00SY*"]):
    responses = responses + xaxis.get_all_responses()

# Print the responses
print(f"------- Start all axis command responses -------")
xaxis.parse_responses(responses)
